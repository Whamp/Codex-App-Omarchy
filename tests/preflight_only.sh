...
assert_contains "$present_output" "Dependency plan: all command-first dependencies are already satisfied."
assert_not_contains "$present_output" "Planned package install command"
...
version_only_omarchy_bin="$version_only_home/.local/share/omarchy/bin"
mkdir -p "$version_only_bin" "$version_only_home" "$version_only_omarchy_bin"
for cmd in pacman curl 7z node pnpm electron; do
  make_fake_cmd "$version_only_bin" "$cmd"
done
make_fake_cmd "$version_only_omarchy_bin" omarchy-version "echo 'Omarchy 2.1.0'"
...
assert_not_contains "$absent_output" "Required system/build packages"
assert_contains "$absent_output" "Planned package install command: sudo bash ./install-codex-omarchy.sh"
